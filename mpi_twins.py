...
def optimize_block_twins(child_block, subm):
    gift_block = subm['GiftId'][child_block].values
    #print (gift_block)
    my_block_size = int(block_size/2)
    C = np.zeros((my_block_size, my_block_size))
    for i in range(my_block_size):
        c1 = int(child_block[i])
        c2 = int(c1+1)
        for j in range(my_block_size):
            g = int(gift_block[j])
            C[i, j] = child_happiness[c1][g] + child_happiness[c2][g]
...
def my_optimizer(subm, score_org, comm, rank,size, gift_data, child_data):
    ...
    while 1:
        ...
        child_blocks = np.split(np.random.permutation
                            (range(triplets, tts-twins_rmd, 2)), n_blocks)
        ## 广播
        ...
        subm_iter = subm_best.copy()
        # for child_block in (child_blocks[start: end]):
#             start_time = dt.datetime.now()
        cids, gids = optimize_block_twins(child_block,subm_iter)
        ...
        for i in range(size):
            subm_iter['GiftId'].loc[buf[i][0]] = buf[i][1]
            subm_iter['GiftId'].loc[buf[i][0] + 1] = buf[i][1]

            # current_gift_ids[buf[i][0]] = buf[i][1]

        # current_gift_ids = comm.bcast(current_gift_ids, root=0)
        # subm['GiftId'] = gift_ids[current_gift_ids]
        answ_iter[subm_iter[['ChildId']].values] = subm_iter[['GiftId']].values
        score_iter = avg_normalized_happiness(subm_iter[['ChildId', 'GiftId']].values, gift_data, child_data)
        # print('Score achieved in current iteration: {:.10f}'.format(score_iter))

        if score_iter > score_best:
            subm_best = subm_iter.copy()
...
#                   format((end_time-start_time).total_seconds()))
        ...
        if rank == 0:
            subm_best[['ChildId', 'GiftId']].to_csv('improved_twins.csv', index=False)
...
block_size = 6000
n_blocks = int(twins / block_size)
twins_rmd = 40000 - n_blocks * block_size
